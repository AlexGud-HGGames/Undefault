...
- A **manual intent timeline** records normalized GSI events and optional `POST /user-actions` entries in one ordered log, with game context on manual rows; see [docs/manual-intent-timeline.md](docs/manual-intent-timeline.md).
...
| `GET` | `/timeline/episodes` | Manual-intent episodes with before/after entry windows |
| `POST` | `/user-actions` | Record manual music intent; apply matching `control-profiles.json` rule |
...
| `Timeline` | Manual intent timeline: enable flag, ring size, JSONL directory, episode window sizes (`TimelineOptions`) |
| `ManualMusicActions` | Gate and optional allowlist for `POST /user-actions` (`ManualMusicActionOptions`) |
| `Keybinds` | Optional Windows global hotkeys that invoke the same path as `POST /user-actions` (`KeybindOptions`; off by default) |
